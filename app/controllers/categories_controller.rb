	...
	def index
		if 	params[:category_id] <= 3.to_s &&  params[:category_id] >= 1.to_s
			...
			if 	params[:category_id] == 1.to_s
				@plant_1 = Plant.where(category_id: 2).order(created_at: "DESC").limit(3)
				@plant_2 = Plant.where(category_id: 3).order(created_at: "DESC").limit(3)
				@category_1 = Category.find_by(id: 2)
				@category_2 = Category.find_by(id: 3)
			elsif
				params[:category_id] == 2.to_s
				@plant_1 = Plant.where(category_id: 1).order(created_at: "DESC").limit(3)
				@plant_2 = Plant.where(category_id: 3).order(created_at: "DESC").limit(3)
				...
				@plant_1 = Plant.where(category_id: 1).order(created_at: "DESC").limit(3)
				@plant_2 = Plant.where(category_id: 2).order(created_at: "DESC").limit(3)
				@category_1 = Category.find_by(id: 1)
